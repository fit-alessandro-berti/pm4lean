...
theorem sequence_child_entry_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
    childEntry addr i ∈ rawPlaces (POWL2.sequence children) addr := by
  have hChild : childEntry addr i ∈ rawPlacesChildren children addr 0 :=
    rawChildEntry_mem children addr hRange
  simpa [rawPlaces] using Or.inr (Or.inr hChild)

theorem sequence_child_exit_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
    childExit addr i ∈ rawPlaces (POWL2.sequence children) addr := by
  have hChild : childExit addr i ∈ rawPlacesChildren children addr 0 :=
    rawChildExit_mem children addr hRange
  simpa [rawPlaces] using Or.inr (Or.inr hChild)

theorem parallel_child_entry_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
    childEntry addr i ∈ rawPlaces (POWL2.parallel children) addr := by
  have hChild : childEntry addr i ∈ rawPlacesChildren children addr 0 :=
    rawChildEntry_mem children addr hRange
  simpa [rawPlaces] using Or.inr hChild

theorem parallel_child_exit_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
    childExit addr i ∈ rawPlaces (POWL2.parallel children) addr := by
  have hChild : childExit addr i ∈ rawPlacesChildren children addr 0 :=
    rawChildExit_mem children addr hRange
  simpa [rawPlaces] using Or.inr hChild
...
theorem sequence_start_transition_mem
    (children : List (POWL2 Activity)) (addr : Address) :
    transition addr TransitionKind.seqStart ∈
      rawTransitions (POWL2.sequence children) addr := by
  by_cases h : children.length = 0
  · simp [rawTransitions, h]
  · simp [rawTransitions, h]

theorem sequence_next_transition_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length - 1) :
    transition addr (TransitionKind.seqNext i) ∈
      rawTransitions (POWL2.sequence children) addr := by
  have hNonempty : children.length ≠ 0 := by
    intro hZero
    rw [hZero] at hRange
    exact Nat.not_lt_zero i hRange
  simp [rawTransitions, hNonempty]
  exact Or.inr (Or.inl ⟨i, hRange, rfl⟩)

theorem sequence_end_transition_mem
    (children : List (POWL2 Activity)) (addr : Address)
...
theorem sequence_start_consumes_entry
    (children : List (POWL2 Activity)) (addr : Address) :
    rawPre (POWL2.sequence children)
      (transition addr TransitionKind.seqStart) (entry addr) = 1 := by
  simp [rawPre, rawMark, entry, transition]

theorem sequence_start_empty_produces_exit
    (children : List (POWL2 Activity)) (addr : Address)
    (hEmpty : children.length = 0) :
    rawPost (POWL2.sequence children)
      (transition addr TransitionKind.seqStart) (exit addr) = 1 := by
  simp [rawPost, rawMark, exit, transition, hEmpty]

theorem sequence_start_nonempty_produces_first_entry
    (children : List (POWL2 Activity)) (addr : Address)
    (hNonempty : children.length ≠ 0) :
    rawPost (POWL2.sequence children)
      (transition addr TransitionKind.seqStart) (childEntry addr 0) = 1 := by
  simp [rawPost, rawMark, childEntry, childAddr, entry, transition, hNonempty]

theorem sequence_next_consumes_child_exit
    (children : List (POWL2 Activity)) (addr : Address) (i : Nat) :
    rawPre (POWL2.sequence children)
      (transition addr (TransitionKind.seqNext i)) (childExit addr i) = 1 := by
  simp [rawPre, rawMark, childExit, childAddr, exit, transition]

theorem sequence_next_produces_next_entry
    (children : List (POWL2 Activity)) (addr : Address) (i : Nat) :
    rawPost (POWL2.sequence children)
      (transition addr (TransitionKind.seqNext i)) (childEntry addr (i + 1)) = 1 := by
  simp [rawPost, rawMark, childEntry, childAddr, entry, transition]

theorem sequence_end_consumes_last_exit
    (children : List (POWL2 Activity)) (addr : Address) :
    rawPre (POWL2.sequence children)
      (transition addr TransitionKind.seqEnd)
      (childExit addr (lastChildIndex children.length)) = 1 := by
  simp [rawPre, rawMark, childExit, childAddr, exit, transition,
    lastChildIndex]

theorem sequence_end_produces_exit
    (children : List (POWL2 Activity)) (addr : Address) :
    rawPost (POWL2.sequence children)
      (transition addr TransitionKind.seqEnd) (exit addr) = 1 := by
  simp [rawPost, rawMark, exit, transition]

theorem parallel_fork_transition_mem
    (children : List (POWL2 Activity)) (addr : Address) :
    transition addr TransitionKind.poFork ∈
      rawTransitions (POWL2.parallel children) addr := by
  by_cases h : children.length = 0
  · simp [rawTransitions, transitionsForPartialOrder, h]
  · simp [rawTransitions, transitionsForPartialOrder, h]

theorem parallel_fork_consumes_entry
    (children : List (POWL2 Activity)) (addr : Address) :
    rawPre (POWL2.parallel children)
      (transition addr TransitionKind.poFork) (entry addr) = 1 := by
  simp [rawPre, rawMark, entry, transition]

theorem parallel_fork_empty_produces_exit
    (children : List (POWL2 Activity)) (addr : Address)
    (hEmpty : children.length = 0) :
    rawPost (POWL2.parallel children)
      (transition addr TransitionKind.poFork) (exit addr) = 1 := by
  simp [rawPost, rawMark, exit, transition, hEmpty]

theorem parallel_begin_transition_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
...
theorem parallel_begin_produces_child_entry
    (children : List (POWL2 Activity)) (addr : Address) (i : Nat) :
    rawPost (POWL2.parallel children)
      (transition addr (TransitionKind.poBegin i)) (childEntry addr i) = 1 := by
  simp [rawPost, rawMark, childEntry, childAddr, entry, transition]

theorem parallel_complete_transition_mem
    (children : List (POWL2 Activity)) (addr : Address)
    {i : Nat} (hRange : i < children.length) :
...
theorem parallel_complete_consumes_child_exit
    (children : List (POWL2 Activity)) (addr : Address) (i : Nat) :
    rawPre (POWL2.parallel children)
      (transition addr (TransitionKind.poComplete i)) (childExit addr i) = 1 := by
  simp [rawPre, rawMark, childExit, childAddr, exit, transition]

theorem parallel_join_transition_mem
    (children : List (POWL2 Activity)) (addr : Address)
    (hNonempty : children.length ≠ 0) :
...
theorem parallel_join_produces_exit
    (children : List (POWL2 Activity)) (addr : Address) :
    rawPost (POWL2.parallel children)
