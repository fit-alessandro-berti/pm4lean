import Pm4Lean.Models.Petri.WFNet.WOFLAN.Boundedness.SelfCover.CutTrace.LargeMarkingThreshold.BadBound.Projection.Coordinates.Failure.NoPredecessor.FiniteBound.Negation

namespace Pm4Lean
namespace Petri

theorem exists_large_retained_noPredecessorGreedyCoordinate_of_not_finiteBound
    {W : WFNet}
    (hNotBound :
              ...
              ∃ coords : List Nat,
                coords ∈ natListGreedyBasis
                  (FiringSequence.prefixCutNoPredecessorSampleCoordinateLists
                    samples) ∧
                  ∃ x : Nat, x ∈ coords ∧ k < x :=
  (not_hasLargeCoveredPrefixCutNoPredecessorGreedyCoordinateFiniteBoundBasis_iff.mp
    hNotBound) k

theorem exists_retained_noPredecessorGreedyCoordinate_above_bounds_of_not_finiteBound
    {W : WFNet}
    (hNotBound :
      ¬ HasLargeCoveredPrefixCutNoPredecessorGreedyCoordinateFiniteBoundBasis
        W)
